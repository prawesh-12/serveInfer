...
inline std::string stateDir() {
  return envValue("EDGE_STATE_DIR");
}

// Every process registers itself as <name>.pid. That directory is the process
// list the dashboard reads and the glob each tier's stop script uses.
inline std::string pidFilePath(const std::string& name) {
