...
class Scheduler {
  ...
  _run(job) {
    ...
    // Released by whichever comes first, the job settling or the timeout. Only
    // in .finally() and a job that never settles holds its slot forever.
    let released = false;
    const releaseSlot = () => {
      if (released) {
        return;
  ...
  // The shell is long-lived, so this map has to be bounded.
  _evictDone() {
