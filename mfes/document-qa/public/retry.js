// The browser's retry policy.
//
// The shell already sends retryAfterSeconds and retryable on every failure it
// thinks is temporary. Before this file existed, nothing read them.
//
// A retry reuses the same requestId on purpose. The api-server keeps successful
// results against that id. So if a retry races a success that arrived late, the
// client gets the original answer back and we do not run the inference twice.
...
// runAttempt(attemptNumber) returns a promise. It resolves when the attempt
// works. When it fails, it rejects with the shell's error payload on
// err.payload, which is what this function reads to decide what to do next.
