...
const port = Number(process.env.CLIENT_PORT || 5003);
...
function serveStatic(req, res) {
  ...
  if (!fs.existsSync(distDir)) {
    send(res, 503, 'chat-dashboard is not built: run "pnpm build" in clients/chat_3\n');
    return;
  }
...
http.createServer(serveStatic).listen(port, '127.0.0.1', () => {
  console.log(`[chat_3] listening on http://127.0.0.1:${port}`);
  console.log(`[chat_3] shell API: ${shellApiBase}`);
});
