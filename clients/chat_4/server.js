...
const port = Number(process.env.CLIENT_PORT || 5004);
...
function serveStatic(req, res) {
  ...
  if (!fs.existsSync(distDir)) {
    send(res, 503, 'chat-dashboard is not built: run "pnpm build" in clients/chat_4\n');
    return;
  }
...
http.createServer(serveStatic).listen(port, '127.0.0.1', () => {
  console.log(`[chat_4] listening on http://127.0.0.1:${port}`);
  console.log(`[chat_4] shell API: ${shellApiBase}`);
});
