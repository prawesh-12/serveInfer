...
function serveStatic(req, res) {
  ...
  if (!fs.existsSync(distDir)) {
    send(res, 503, 'chat-dashboard is not built: run "pnpm build" in clients/chat_1\n');
    return;
  }
...
http.createServer(serveStatic).listen(port, '127.0.0.1', () => {
  console.log(`[chat_1] listening on http://127.0.0.1:${port}`);
  console.log(`[chat_1] shell API: ${shellApiBase}`);
});
